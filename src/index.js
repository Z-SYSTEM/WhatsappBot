...
import { setupWebUI } from './web/web-server.js';
...
async function startServer() {
  // Crear servidor HTTP único
  const server = http.createServer(app);
  
  // Configurar interfaz web UI (incluye Socket.IO)
  const io = setupWebUI(app, server, config);
  ...
  // Configurar rutas de la API DESPUÉS de inicializar el bot
  ...
  server.listen(config.port, () => {
    ...
    logger.info(`API REST disponible en: http://localhost:${config.port}/api`);
    logger.info(`Interfaz web disponible en: http://localhost:${config.port}`);
  ...
  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
      logger.error(`[SERVER] El puerto ${config.port} ya está en uso por otra instancia`);
